function frame = fn_generate_dummy_frame_data(N_Tm, N_Am, N_Fm, N_Bm_max, N_Pm, raw_data_type)
...
frame.probe_placement_index = int64(zeros(N_Am, N_Fm));
for ii = 1:N_Fm
    if ii == 1
        jj = [1:N_Bm(ii)];
    else
    ...
    frame.probe_placement_index(:, ii) = jj(randi(N_Bm(ii), N_Am, 1));
end
